...
import com.app.firebaselistsearch.RealtimeDbActivity
...
class MyUserAdapter(val usersList: List<MyUser>, val context : Context, val activity: RealtimeDbActivity): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (itemFlag != 0) {
            ...
            viewHolder.title!!.text = "${usersList[position].first_name} ${usersList[position].last_name}"
            viewHolder.desc!!.text = usersList[position].email
            val thumbnailUrl = usersList[position].avater
...
//                emptyViewHolder.emptyImage.setImageResource(R.drawable.ic_notification)
    ...
    interface UpcomingAdapterListener {
        ...
        fun onListItemClicked(dummyData: MyUser,position: Int)
